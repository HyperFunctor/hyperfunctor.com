...
const InvoiceTemplate = ({ company_name, street_address, city, postal_code, nip, bundle }) =>
  ({
    invoice: {
...
const handler: NextApiHandler = async (req, res) => {
  ...
  try {
    ...
    const session: Stripe.Checkout.Session = await stripe.checkout.sessions
      .retrieve(id, { expand: ["payment_intent"] },);

    const payment_intent: Stripe.PaymentIntent = session.payment_intent as Stripe.PaymentIntent;
    ...
    if (method !== "POST") {
      ...
      const method = 'POST';
      const headers = {"X-inFakt-ApiKey": APIKEY, "Content-Type": "application/json"}
      const body = JSON.stringify(InvoiceTemplate(order));
      const r = await fetch(`https://api.infakt.pl/v3/invoices.json`, { method, body, headers });
